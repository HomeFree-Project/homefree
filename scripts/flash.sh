...
Flash a NixOS image to an SD card or USB device.
...
Options:
  ...
  -b, --build-dir DIR       Build directory containing images (default: ./result/iso in flake dir)
  -i, --image FILE          Specific image file to flash (overrides auto-detection)
  -c, --config NAME         Configuration name to flash (e.g., homefree)
...
Examples:
  ...
  $SCRIPT_NAME -i ./custom-image.iso /dev/sdb
...
if [[ -n "$IMAGE_FILE" ]]; then
    ...
    IMAGE_TO_FLASH="$BUILD_DIR/${CONFIG_NAME}.iso"
    if [[ ! -f "$IMAGE_TO_FLASH" ]]; then
        ...
        ls -la "$BUILD_DIR"/*.iso 2>/dev/null || echo "  No .iso files found"
        exit 1
    fi
else
    ...
    # Find .iso files in build directory
    mapfile -t IMAGES < <(find "$BUILD_DIR" -maxdepth 1 -name "*.iso" -type f 2>/dev/null)

    if [[ ${#IMAGES[@]} -eq 0 ]]; then
        log_error "No .iso files found in $BUILD_DIR"
        log_error "Please build an image first using build-image.sh"
        exit 1
...
if [[ "$HAS_PV" == "true" ]]; then
    ...
    if dd if="$IMAGE_TO_FLASH" bs=4M | pv -s "$IMAGE_SIZE" | $SUDO dd of="$DEVICE" bs=4M oflag=dsync status=none; then
...
log_info "You can now safely remove the device and use it to boot."
