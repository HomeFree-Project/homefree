# From: https://github.com/jpetrucciani/nix/blob/main/mods/pkgs/zaddy.nix
final: prev:
with prev;
rec {
  caddy-with-plugins-builder =
    let
      builtins = {
        ...
        ## Requires go 1.24, which requires Nix Unstable
    ...
    let
      ...
      caddy-with-plugins =
        ...
        in
        prev.caddy.overrideAttrs (oldAttrs: {
          inherit version src;
          inherit vendorHash;
          ...
          passthru = oldAttrs.passthru // {
            overrideModAttrs = finalModAttrs: previousModAttrs: {
              preBuild = ''
