...
from resolvers.source_iso import router as source_iso_router
...
app.include_router(source_iso_router)
...
# Apps generated by the user's own AI builder (ai.homefree.host) live
# under this dir, one project per subdir, each with a manifest.json.
AI_PROJECTS_DIR = "/var/lib/homefree-ai-podman/data/projects"
...
def _ai_built_service_labels() -> set:
    """Live service labels of apps built by ai.homefree.host (flake mode).

    Each AI project's manifest.json carries `service_label`, which the
    builder keeps current across subdomain renames — so it is the
    rename-proof join from an AI project to its live dashboard tile (the
    compiled service-config carries no flake-source field, and the label
    desyncs from the plugins.flakes slug on rename). `dockerfile`-mode
    projects are served by the AI meta-proxy, are not NixOS services, and
    never reach this dashboard, so they're skipped. Best-effort: a missing
    dir or unreadable/garbled manifest is ignored, never fatal.
    """
    import os
    import glob
    import json
    labels = set()
    try:
        for mf in glob.glob(os.path.join(AI_PROJECTS_DIR, "*", "manifest.json")):
            try:
                with open(mf) as f:
                    m = json.load(f)
            except Exception:
            ...
            if m.get("mode") != "flake":
                continue
            lbl = (m.get("service_label") or "").strip()
            if lbl:
                labels.add(lbl)
    except Exception:
        pass
...
async def get_services_visible_to_me(request: Request):
    ...
    Returns {label, name, project_name, url, icon, access, category,
    description, host} per entry — a deliberately narrow shape, no
    systemd state, no config internals. `access` is "sso", "public", or
    "lan" (see badge logic below). `category`/`description` drive the
    home-dashboard grouping + tile tagline; the user's AI-built apps are
    force-grouped into "My Apps". The admin UI uses /api/services for the
    fuller picture.
    """
    import json
    groups = getattr(request.state, "auth_groups", set()) or set()
    ...
    # Labels of the user's own AI-built apps — pinned into their own
    ...
    for entry in cfg.get("services", []):
        ...
        # Per-app "Hide from dashboard" state (App Configuration page).
        # The operator's choice lives in homefree-config.json; when unset,
        # fall back to the app's catalog default (hidden-default, true only
        # for OpenSprinkler UI). Hidden apps are still RETURNED (with the
        # `hidden` flag) so the dashboard's "Show hidden" toggle can reveal
        # them; the frontend filters them out by default.
        # See services/admin-web/dashboard-catalog.nix.
        svc_json = (user_cfg.get("services") or {}).get(label, {})
        hidden = svc_json.get("hide-from-dashboard")
        if hidden is None:
            hidden = sc.get("hidden-default", False)
        hidden = bool(hidden)
        ...
        # Host shown under the title (e.g. "photos.homefree.host").
        try:
            from urllib.parse import urlparse
            host = urlparse(url).hostname or ""
        except Exception:
        ...
        # Dashboard group. The app declares its own `category`; the
        # user's AI-built apps are force-grouped into "My Apps".
        category = sc.get("category") or ""
        if label in ai_labels:
            category = AI_APPS_CATEGORY
