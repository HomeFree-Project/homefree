...
- **Modern UI** built with Lit web components
- **REST API** backend for installation management
...
> For the full architecture (admin mode, modules, save & apply workflow), see
> [`ARCHITECTURE.md`](./ARCHITECTURE.md).

## Architecture

### Frontend (`frontend/`)
- **Framework**: Lit (lightweight, standards-based web components), vendored under
  `frontend/src/vendor/` for offline ISO builds
- **API Client**: REST via `frontend/src/api/client.js`
- **Build Tool**: Vite
- **Components**:
  - Welcome screen
...
### Backend (`backend/`)
- **Framework**: FastAPI exposing a REST API (`simple_main.py` is the production entry point)
- **Language**: Python 3
- **Services**:
...
python simple_main.py  # Starts the REST API server on :8000
```
...
The backend exposes a REST API consumed exclusively through
`frontend/src/api/client.js`. Representative endpoints:

- `GET /api/mode` - Current surface (installer | admin)
- `GET /api/system` - Hardware / locale detection
...
- `POST /api/config/preview` - Preview via `nixos-rebuild dry-activate`
- `POST /api/config/apply` - Apply changes with rebuild
- `GET /api/config/rebuild-status` - Monitor rebuild progress
- `POST /api/install/start` - Begin installation (installer mode)

See `ARCHITECTURE.md` for the full backend resolver map.
...
Progress is streamed to the frontend by polling the REST status endpoint.
...
1. Add or update REST routes in `backend/simple_main.py`
2. Implement request handlers in `backend/resolvers/`
