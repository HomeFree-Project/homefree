  ...
  options.homefree.service-options.zitadel = {
    ...
    secrets = {
      env = lib.mkOption {
        ...
        default = null;
        description = ''
          Optional manual override: path to an env file passed to the
          Zitadel container. Normally unused — the masterkey, admin
          password, and OAuth2 Proxy secrets are all managed by
          zitadel-prepare-secrets.service / zitadel-provision.service /
          the web installer. Set this only if you need to inject extra
          ZITADEL_* env vars (e.g. SMTP, custom logging).
        '';
      };

      # The three secrets below are internal — they're populated
      # automatically (cookie-secret by zitadel-prepare-secrets, the two
      # OIDC fields by zitadel-provision) so they should not appear as
      # user-fillable fields in the admin UI's SSO entry.
      oauth2-cookie-secret = lib.mkOption {
        type = lib.types.nullOr lib.types.str;
        default = null;
        internal = true;
        description = "(internal) auto-generated by zitadel-prepare-secrets.service.";
      };
      oauth2-client-id = lib.mkOption {
        type = lib.types.nullOr lib.types.str;
        default = null;
        internal = true;
        description = "(internal) written by zitadel-provision.service after creating the homefree-oauth2proxy OIDC application.";
      };
      oauth2-client-secret = lib.mkOption {
        type = lib.types.nullOr lib.types.str;
        default = null;
        internal = true;
        description = "(internal) written by zitadel-provision.service alongside the client ID.";
      };
    };
  };
        ...
        zitadel = {
          ...
          environment = {
            ...
            ZITADEL_FIRSTINSTANCE_ORG_HUMAN_EMAIL_ADDRESS =
              "${config.homefree.system.adminUsername}@${config.homefree.system.domain}";
          ...
          ## with mode 600 even when it would be empty). User overrides
          ## from secrets.env are appended; later entries win on conflict.
          environmentFiles = [ zitadelEnvFile ]
            ++ lib.optional (zitadelSecrets.env != null) zitadelSecrets.env;
        };
